    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-gray-200 dark:border-white/[0.06] bg-gray-50 dark:bg-white/[0.02] p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            onClick={onTaskAdd}
            className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-medium bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-200 border border-indigo-200 dark:border-white/10 rounded-lg hover:bg-indigo-500/30 transition-all cursor-pointer"
          >
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] uppercase tracking-wider text-gray-600 font-semibold">Sort by</span>
          {(["priority", "dueDate", "title", "status"] as SortBy[]).map((opt) => (
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-[11px] font-medium border transition-all cursor-pointer ${
                sortBy === opt
                  ? "bg-indigo-50 dark:bg-indigo-500/15 text-indigo-600 dark:text-indigo-300 border-indigo-200 dark:border-indigo-500/20"
                  : "bg-gray-50 dark:bg-white/[0.02] text-gray-500 border-gray-200 dark:border-white/[0.04] hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/[0.04]"
              }`}
            >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs border transition-all cursor-pointer ${
            showDone
              ? "bg-gray-50 dark:bg-white/[0.03] text-gray-600 dark:text-gray-400 border-gray-200 dark:border-white/[0.06]"
      ...
      <div className="space-y-1.5">
        {sorted.length === 0 ? (
          ...
          sorted.map((task) => (
            <div
              key={task._id}
              className="group flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-200 dark:border-white/[0.04] hover:border-gray-300 dark:hover:border-white/[0.08] bg-gray-50 dark:bg-white/[0.01] hover:bg-gray-100 dark:hover:bg-white/[0.03] transition-all"
            >
              {/* Checkbox */}
              <button
              ...
              <button
                ...
                className={`flex-1 text-left text-sm transition-colors cursor-pointer min-w-0 ${
                  task.status === "done"
                    ...
                    : "text-gray-900 dark:text-white hover:text-indigo-600 dark:hover:text-indigo-200"
                }`}
              >
